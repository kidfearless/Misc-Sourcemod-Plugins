...
void UpdateHUD(int client)
{
	...
	else if((gI_HUDSettings[client] & HUD_CENTER) > 0)
	{
		...
		if(!IsFakeClient(target))
		{
			...
			if(gEV_Type == Engine_CSGO)
			{
				strcopy(sHintText, 512, "<font size='17' face=''>");
				...
				if(status >= Timer_Running)
				{
					...
					if(track != Track_Main)
					{
						Format(sHintText, 512, "%s[<font color='#FFFFFF'>B</font>] ", sHintText);
					}

					Format(sHintText, 512, "%s<font color='#%s'>%s</font> (%d)\t%s %s %s", sHintText, sColor, sTime, rank, (buttons & IN_JUMP) > 0? "J":"_", (buttons & IN_FORWARD) > 0? "W":"_", (buttons & IN_DUCK) > 0? "C ":"_");
				}
				...
				{
					char asd[8];
					Format(asd, sizeof(asd), "%s %s %s", (buttons & IN_MOVELEFT) > 0? "A":"_", (buttons & IN_BACK) > 0? "S":"_", (buttons & IN_MOVERIGHT) > 0? "D":"_");
					

					Format(sHintText, 512, "%s\n%T: %d%s%s\t%T: <font color='#%s'>%s</font>", sHintText, "HudJumpsText", client, jumps, (jumps < 1000)? "\t":"", asd, "HudStyleText", client, gS_StyleStrings[style][sHTMLColor], gS_StyleStrings[style][sStyleName]);
				}
...
void UpdateCenterKeys(int client)
{
	...
	FormatEx(sCenterText, sizeof(sCe), "　%s　　%s\n　　 %s\n%s　 %s 　%s", 
		(buttons & IN_JUMP) > 0? "Ｊ":"ｰ", (buttons & IN_DUCK) > 0? "Ｃ":"ｰ",
		(buttons & IN_FORWARD) > 0? "Ｗ":"ｰ", (buttons & IN_MOVELEFT) > 0? "Ａ":"ｰ",
		(buttons & IN_BACK) > 0? "Ｓ":"ｰ", (buttons & IN_MOVERIGHT) > 0? "Ｄ":"ｰ");
