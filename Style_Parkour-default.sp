...
const char FSOLID_NOT_SOLID = 0x0004;
const char FSOLID_TRIGGER = 0x0008;
...
float g_fRocketMins[] = {-0.0, -0.0, -0.0};
float g_fRocketMaxs[] = {0.0, 0.0, 0.0};
...
#define GRAPPLE_PULL_ACCEL 32.0
#define GRAPPLE_CHARGE_MAX 1024.0
#define GRAPPLE_CHARGE_USE -4.272461
#define GRAPPLE_CHARGE_REGEN 1.28
...
public void OnActivated(const int& iClient)
{
...
public void OnDeactivated(const int& iClient)
{
	g_eParkourData[iClient][Parkour_Enabled] = false;
...
bool IsNearWall(const int& iClient)
{
...
public void OnPreThinkPost(int iClient)
{
	...
	OnGrapple(iClient, (iButtons & IN_ATTACK == IN_ATTACK));
...
void OnGrapple(const int& iClient, const bool bState)
{
...
int CreateHook(const int& iClient)
{
...
void KillHook(const int& iClient)
{
	int iHook = EntRefToEntIndex(g_eParkourData[iClient][Grapple_HookEntityRef]);
	if(iHook > 0)
...
void HookCharge(const int& iClient, const float fAmount)
{
	if(GetEntityFlags(iClient) & FL_ONGROUND && fAmount > 0.0)
	{
		g_eParkourData[iClient][Grapple_Charge] += 12.8 * fAmount;
	}
	else
	{
...
void GetChargeColor(const int& iClient, int iResult[4])
{
...
void InitHook(const int& iClient, const int& iHook)
{
	DispatchSpawn(iHook);
	
	SetEntityMoveType(iHook, MOVETYPE_FLYGRAVITY);
	SetEntProp(iHook, Prop_Send, "m_CollisionGroup", COLLISION_GROUP_PLAYER_MOVEMENT);
	...
	GetEntPropVector(iHook, Prop_Data, "m_vecAbsVelocity", fAbsVelocity);

	ScaleVector(fAbsVelocity, 5.0);
	SetEntPropVector(iHook, Prop_Data, "m_vecAbsVelocity", fAbsVelocity);
...
void OnAttack2(const int& iClient)
{
