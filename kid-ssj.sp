...
public void OnClientCookiesCached(int client)
{
	...
	GetClientCookie(client, gH_CookieTimerStatus, sCookie, 8);
	gB_TimerStatus[client] = view_as<bool>(StringToInt(sCookie));
	
	GetClientCookie(client, gH_CookieSpeedLoss, sCookie, 8);
	gB_SpeedLoss[client] = view_as<bool>(StringToInt(sCookie));
	
	GetClientCookie(client, gH_CookieStrafeCount, sCookie, 8);
...
public void Player_Jump(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsFakeClient(client) ||
		(gI_Jump[client] > 0 && gI_StrafeTick[client] <= 0))
	{
		return;
	}
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && GetHUDTarget(i) == client && gB_Enabled[i])
		{
			if(SSJ_PrintStats(i, client) || gI_Jump[client] == 1)
			{
				UpdateStats(i);
			}
		}
...
{
	if(gI_OldSSJTarget[target] != client)
	{
		UpdateStats(target);
		gI_OldSSJTarget[target] = client;
	}
	...
	if(gI_Jump[target] > 1)
	{
		...
		{
			Format(sMessage, 192, "%s %s| Speed Δ: %s%i", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable2], RoundToFloor(GetVectorLength(velocity)) - RoundToFloor(gF_OldSpeed[client]));
		}
...
void PrintToClient(int client, const char[] message, any ...)
{
	...
	{
		PrintToChat(client, "%s%s%s%s", (gEV_Type == Engine_CSGO)?" ":"", gS_ChatStrings[sMessagePrefix], gS_ChatStrings[sMessageText], buffer);
	}
}
